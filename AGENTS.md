...
│   └── PitCoverageAnnotator.kt       # Project service: parses report + annotates open editors (band + gutter icon w/ hover tooltip)
...
- The console "Open report in browser" link prefers the top-level `reportDir/index.html` (PIT 1.25.8 layout) and falls back to the latest timestamped subdirectory for older layouts. `DirectoryReader.getLatestDirectoryFrom` only returns subdirectories, so it must NOT be used directly to build the link for top-level reports.
