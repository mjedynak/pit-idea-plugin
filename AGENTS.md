...
- **Report wait logic**: each test cleans `report/` (with retry, `cleanReportDir`) before running PIT, then `waitForHtmlReport` waits for the top-level `report/index.html` (or a new 14-digit timestamped subdir as a fallback) up to 30s. The report is NOT deleted after a run to avoid racing with the plugin's `processTerminated` handler that writes `pit-output.log` — the next test cleans it instead.
