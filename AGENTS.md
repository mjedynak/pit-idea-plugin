...
- **Report wait logic**: each test cleans the resolved report dir (with retry, `cleanReportDir`) before running PIT, then `waitForHtmlReport` waits for the top-level `index.html` (or a new 14-digit timestamped subdir as a fallback) up to 30s. The report dir is resolved by `expectedReportDir()` mirroring `DefaultArgumentsContainerPopulator.addReportDir()` — since the test project contains `build.gradle.kts` it is detected as a Gradle project and the report goes to `build/testProject/build/reports/pit` (NOT `report/`). The report is NOT deleted after a run to avoid racing with the plugin's `processTerminated` handler that writes `pit-output.log` — the next test cleans it instead.
...
- `PitOutputReader.kt` (in `src/testSupport/`) — reads PIT process info from `RunContentManager` via reflection (exit code, command line, report dir contents, and optionally `pit-output.log` written by `ProcessAdapter`). It resolves the report dir through the plugin's own `MavenProjectDeterminer`/`GradleProjectDeterminer` + `DefaultArgumentsContainerPopulator` constants, so diagnostics always point at the directory the plugin actually wrote to.
- `PitPluginIntegrationTest.kt` — `@Remote` stubs call `PitTestHelper`/`PitActionTestHelper`/`PitOutputReader` inside the test IDE over JMX. Both test methods share a single IDE process (started once in `@BeforeAll`, closed in `@AfterAll`). Each test cleans the resolved report dir (via `expectedReportDir()`, mirroring the plugin's Maven/Gradle/plain detection), runs PIT, waits for a NEW report (top-level `index.html` or a fresh timestamped subdir), asserts via `assertReportFiles()`, and leaves the report for the next test to clean. `waitForHtmlReport` never matches pre-existing/stale reports (they'd cause flaky false-passes or partial-report assertions). Failure messages include project-type detection inputs, report dir contents, any `index.html` files found under the project, and the full PIT process output.
